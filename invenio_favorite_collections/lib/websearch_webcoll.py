...
""" CDS hack here """
from  invenio.websearch_favorite_collection import perform_calculate_favorites
...
def perform_display_collection(uid, colID, colname, aas, ln, em, show_help_boxes):
    ...
    # meanwhile, 
    # tmpl_narrowsearch prepares the HTML for focuson, so similarly, pass a fav_data_object which adds some <div> for hovercards for each collection
    # when deletion required, (a cross displayed in front of favorites), an AJAX request passed to webinterface file, (in req) and collID and uID is passed. this gets passed to websearch_favorites which calls deleted method and returns the new favorites tuple. this return tuple, when bibsched webcoll is run, updates the display

    """ CDS hack begins here """
    fav_data = perform_calculate_favorites(uid)
    fav_body = websearch_templates.tmpl_favorite_collection(fav_data, ln, aas)

    c_body = websearch_templates.tmpl_webcoll_body(ln, colID, fav_body, data["te_portalbox"],
