...
<moz:SearchForm>%(CFG_SITE_URL)s</moz:SearchForm>
  ...
  {'CFG_SITE_URL': CFG_SITE_URL,
   ...
   'html_search_syntax': self.tmpl_opensearch_html_url_syntax
    ...
    """ CDS hack BEGINS here """
    def tmpl_webcoll_body(self, ln, collection, fav_body, te_portalbox,
        ...
        if show_body:
            ...
            #favorites.append(res("SELECT name FROM collection WHERE id=%d"%(id_fav[0][0]))[0][0])
            if focuson:
    ...
    def tmpl_favorite_collection(self, fav_data, ln, aas, show_body=True):
        """Adds to the body of main search page, a column named 'Your Favorite Collection(s)'
        
        Parameters:
        
          - fav_data - Tuples containing the favorites for a user
        """
        _ = gettext_set_language(ln)
        ...
        out = ""
        out +=''' <script type="text/javascript" src="http://code.jquery.com/jquery-1.8.3.js"></script>
  ...
  <script type="text/javascript">
    ...
    $('.hovercontent').fadeIn().css({ 'top': pos.top - 5 + 'px', // subtract 5px to account for padding
    'left': pos.left + 80 + 'px'
    });    
    }, 50);
    }, function () {
...
'''
        out +='''
  <div class="hovercontent">Remove this collection from favorites?
    <input id="answer" type='button' value='Yes'>
  </div>'''
        out += '''<thead><table class="favcollections"> <tr>
                       <th align="left" colspan="2"> %(favorite)s </th>
                       </tr> </thead><tbody><ul>'''%{ 'favorite':_("Your favorite collections:") }
        if fav_data:
            if type(fav_data)==list:
                for fav in fav_data:
                    out += """<tr><td class="%(favcollections)sbody" valign="top"><li>"""
                    out += """<td class="profile" valign="top">%(link)s """ % {
                        'link': create_html_link(self.build_search_interface_url(c=fav, ln=ln, aas=aas),
                                                 {}, cgi.escape(fav) )}
                    out += """</li></td></tr>"""
            else:
                out += '''<tr><td class="%(favcollections)sbody" valign="top">'''+fav_data+'''</td></tr>'''
        else:
            out += '''<tr><td class="%(favcollections)sbody" valign="top">..contains 0 items </td></tr>'''
        out += "</ul></tbody></table>"
        return out
    
    def tmpl_search_query_viz(self, show_body=True):
        """ Prepares the body for visualizations """
...
"""
        ...
        for son in sons:
            ...
            # tooltip HACK
            out += """<td class="tooltip" title="Add to favorites" valign="top">%(link)s%(recs)s """ % {
            ...
            if display_grandsons and len(grandsons[i]):
                ...
                for grandson in grandsons[i]:
                    out += """ <small class="tooltip" title="Add me to favorites">%(link)s%(nbrec)s</small> """ % {
    ...
    def tmpl_author_information(self, req, pubs, authorname, num_downloads,
        ...
        """
        from invenio.search_engine import request_search
